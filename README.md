...
* `/api/recommend`: Receives two parameters: `time` and `category`, indicating the time frame (`10:00-12:00` for example), and the category of the activity respectively. The day of the week will be automatically deducted using the current date. This call returns either an activity according to the problem statement, or a HTTP response with code 404 in case no activity can be recommended according to the parameters defined.
